...
\begin{rmk}
    ...
    \end{align*}
    To find the coefficient of $u^{(k-1)}\bar{v}^{(j-1)}$, we need to fix $k$ and collect the above coefficient across all values of $m$. Since $m$ goes up to $n$, $m-k$ goes up to $n-k$. But $\binom{x}{j-1}=0$ for $x<j-1$, so $m-k$ effectively ranges from $j-1$ to $n-k$. Let $l':=m-k$, then $m=k+l'$, and the above equation becomes
    \begin{align*}
