...
\href[page=11]{./literature/Constructing adjoint boundary conditions/Giles-Pierce2000_Article_AnIntroductionToTheAdjointAppr.pdf}{For $L$ with lower orders, the adjoint boundary condition can be found using integration by parts.}
...
P = \begin{bmatrix} 1 & 0\\0 & 0 \end{bmatrix} \quad 
Q = \begin{bmatrix} 0 & 0\\ 0 & 1\end{bmatrix}.\]
The matrix $B(t)$ in this case would be the $2\times 2$ matrix associated with $[xy](t)$, which, by Theorem \ref{thm:green's formula}, is given by
\begin{align*}
    [xy](t) &= \sum_{m=1}^2\sum_{j+k=m-1}(-1)^j x^{(k)}(t)\bar{y}^{(j)}(t)\\
    &= (-1)^0x(t)\bar{y}(t) + (-1)^0x^{(1)}(t)\bar{y}(t) + (-1)^1x(t)\bar{y}^{(1)}(t)\\
    &= x(t)\bar{y}(t) + x'(t)\bar{y}(t) - x(t)\bar{y}'(t)\\
...
\end{bmatrix}.
\]
In this case, since all $p_k=1$, $B(0)=B(1)$.
We verify that
\begin{align*}
    ...
    \begin{bmatrix} 1 & 0\\0 & 0 \end{bmatrix}\\
        &= N B^{-1}(1) Q.
\end{align*}
So $U^+x=0$ is indeed adjoint to $Ux=0$. 

\subsection{\href[page=2]{./literature/Constructing adjoint boundary conditions/AIAA-97-1850.pdf}{A paper} that mentions construction}
\todo[inline]{TBC. The paper still discusses a lot about existence; but perhaps can help narrowing down the search.}
\end{document}
