...
Let $\hat{B}$ be as in \ref{eq:eq:green's formula in semibilinear form} which depends on $L$. By Proposition \ref{prop:unique nonsingular G in semibilinear form}, $J=(\hat{B}H^{-1})^*$ is a nonsingular $2n\times 2n$ matrix such that $\mathcal{S}(f,g) = \hat{B}f\cdot g = Hf\cdot Jg$. Take $U^+y$ to be the last $n$ rows of $Jg$. Then we have found $U^+$.
