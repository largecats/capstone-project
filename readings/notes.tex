...
\href[page=11]{./literature/Constructing adjoint boundary conditions/Giles-Pierce2000_Article_AnIntroductionToTheAdjointAppr.pdf}{For $L$ with lower orders, the adjoint boundary condition can be (practically) found using integration by parts.}
...
Let $\mathcal{V}$ denote the vector space of boundary operators over $\C$ with dimension $2n$.\unsure[inline]{Is this how $\mathcal{V}$ should be characterized? Should I prove that it's a vector space?} Then $U=(U_1,\ldots,U_n)$ is a list of $n$ linearly independent vectors in $\mathcal{V}$. Find a basis for $\mathcal{V}$ by building a maximally linearly independent set adding one vector at a time.\unsure[inline]{Is this a (computationally) suitable way to find a basis?} By \href[page=58]{./textbooks/Linear Algebra Done Right 3rd ed.pdf}{a result from linear algebra}, using this basis, $U_1,\ldots,U_n$ can be extended to $U_1,\ldots,U_{2n}$, a basis of $\mathcal{V}$. Thus, we have found $U_c=(U_{n+1},\ldots,U_{2n})$.
...
By Proposition \ref{prop:existence of G}, there exists unique $2n\times 2n$ nonsingular $J=(\hat{B}H^{-1})^*$ such that $\mathcal{S}(f,g) = \hat{B}f\cdot g = Hf\cdot Jg$. Take $U^+y$ to be the last $n$ rows of $Jg$. Then we have found $U^+$.
