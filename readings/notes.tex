...
\begin{rmk}
    ...
    \begin{equation}\label{eq:[uv](t) in B matrix}
        \begin{split}
            ...
            &= \sum_{m=1}^n\sum_{k=1}^{m}(-1)^{m-k}\left(\sum_{l=0}^{m-k}\binom{m-k}{l}p_{n-m}^{(m-k-l)}(t)\bar{v}^{(l)}(t)\right)u^{(k-1)}(t)\\
            % &= \sum_{j+k=1}^n\sum_{k=1}^{j+k}(-1)^{j+k-k}\left(\sum_{l=0}^{j+k-k}\binom{j+k-k}{l}p_{n-j-k}^{(j+k-k-l)}(t)\bar{v}^{(l)}(t)\right)u^{(k-1)}(t)\\
            % &= \sum_{j+k=1}^n\sum_{k=1}^{j+k}(-1)^{j}\left(\sum_{l=0}^{j}\binom{j}{l}p_{n-j-k}^{(j-l)}(t)\bar{v}^{(l)}(t)\right)u^{(k-1)}(t)\\
    ...
    To find $B_{jk}$, we want to pull out the coefficients of $u^{(k-1)}\bar{v}^{(j-1)}$. Fixing $m$ and $k$, we note that when $l=j-1$, the coefficient of $\bar{v}^{(j-1)}(t)$ is
    \begin{align*}
        \binom{m-k}{j-1}p_{n-m}^{(m-k-j+1)}(t).
    \end{align*}
    Now we collect the above coefficient across all values of $m$ and $k$. Since $m$ goes up to $n$, $m-k$ goes up to $n-k$. But $\binom{x}{j-1}=0$ for $x<j-1$, so $m-k$ effectively ranges from $j-1$ to $n-k$. Let $l':=m-k$, then $m=k+l'$, and the above equation becomes
    \begin{align*}
        [uv](t) &= \sum_{m=1}^n\sum_{k=1}^{m}(-1)^{m-k}\left(\sum_{l=0}^{m-k}\binom{m-k}{l}p_{n-m}^{(m-k-l)}(t)\bar{v}^{(l)}(t)\right)u^{(k-1)}(t)\\
        &= \sum_{k=1}^n\sum_{j=1}^n (-1)^{l'}\left(\sum_{l'=j-1}^{n-k}\binom{l'}{j-1}p_{n-(k+l')}^{(l'-(j-1))}\right)\bar{v}^{(j-1)}(t)u^{(k-1)}(t)\\
        &= \sum_{j,k=1}^n \left(\sum_{l=j-1}^{n-k}\binom{l}{j-1}p^{(l-j+1)}_{n-k-l}(-1)^l\right)u^{(k-1)}(t)\bar{v}^{(j-1)}(t).
    \end{align*}
    Thus,
    \[B_{jk} = \sum_{l=j-1}^{n-k}\binom{l}{j-1}p^{(l-j+1)}_{n-k-l}(-1)^l.\]
    }
    \unsure[inline]{Does this argument make sense?}
