...
\begin{defn}
Let $L$ be the \href[page=296]{./textbooks/[Earl_A_Coddington_Norman_Levinson]_Theory_of_Ordinary_Differential_Equations.pdf}{\textbf{linear differential operator of oder $n$ ($n\geq 1$)}} on $[a,b]$ defined by
...
\end{thm}
\begin{rmk}
    \href[page=297]{./textbooks/[Earl_A_Coddington_Norman_Levinson]_Theory_of_Ordinary_Differential_Equations.pdf}{Alternatively}, $[uv](t)$ can be written as 
    %\todo[inline]{Find an explicit way to write $B_{jk}$ in terms of $p_{n-m}$. I'm thinking of deducing a formula for $B_{jk}$ by checking $n=2, 3, 4$, or implementing a program in Julia to figure out the expression for $B_{jk}$.}
    % \todo[inline]{Prove this formula}
    \begin{equation}\label{eq:[uv](t) in B matrix}
        ...
        \end{split}
    \end{equation}
    for some $B_{jk}$ to be found later.
    ...
    To find $B_{jk}$, we need to pull out the coefficients of $u^{(k-1)}\bar{v}^{(j-1)}$. We first note that, fixing $m$ and $k$, when $l=j-1$, the coefficient of $\bar{v}^{(j-1)}$ is
    \begin{align*}
        \binom{m-k}{j-1}p_{n-m}^{(m-k-j+1)}(t).
    \end{align*}
    To find the coefficient of $u^{(k-1)}\bar{v}^{(j-1)}$, we need to fix $k$ and collect the above coefficient across all values of $m$. Since $m$ goes up to $n$, $m-k$ goes up to $n-k$. Since $l\leq m-k$, $l=j-1$ implies $j-1\leq m-k$. Thus, $m-k$ ranges from $j-1$ to $n-k$.
    % But $\binom{x}{j-1}=0$ for $x<j-1$, so $m-k$ effectively ranges from $j-1$ to $n-k$. 
...
is nonsingular (because it has inverse $\begin{bmatrix}M & N\\ M_c & N_c\end{bmatrix}$), i.e., it has full rank. Thus, if 
    ...
    \LIN{Since $A^\star$ is a linear map, $U^+y=0$ implies $A^*U^+y=0$.}
...
Let $\hat{B}$ be as in Theorem \ref{thm:boundary form formula} which depends on $L$. By Proposition \ref{prop:unique nonsingular G in semibilinear form}, $J=(\hat{B}H^{-1})^*$ is a nonsingular $2n\times 2n$ matrix such that $\mathcal{S}(f,g) = \hat{B}f\cdot g = Hf\cdot Jg$. Take $U^+y$ to be the last $n$ rows of $Jg$. Then we have found $U^+$.
...
\newpage
\section{Smith-Fokas paper}
\subsection{Implement the Fokas transform pairs (2.15a, 2.16a)}
\subsubsection{Chebyshev polynomial approximation of functions on intervals}
...
\subsubsection{Integral of Chebyshev polynomial}
Suppose that on $[0,1]$,
\begin{align*}
...
\frac{n}{n^2-1}\sin(\frac{\pi}{2})\sin(\frac{n\pi}{2}) + \frac{1}{n^2-1}\cos(\frac{\pi}{2})\cos(\frac{n\pi}{2})\\ 
- \frac{n}{n^2-1}\sin(0)\sin(0) - \frac{1}{n^2-1}\cos(0)\cos(0)&\quad\mbox{if $n\geq 2$}
\end{cases}\\
&= 
\begin{cases}
1&\quad\mbox{if $n=0$}\\
...
&= \frac{1}{ic}\left(n\int_0^{\frac{\pi}{2}}e^{-ic\cos\theta}\sin(n\theta)\,d\theta + \cos(\frac{n\pi}{2})- e^{-ic}\right).
\end{align*}
...
&= \begin{cases}
    \frac{1}{z}e^z - \frac{n}{z}K_n(z)&\quad\mbox{if $n$ odd}\\
    \frac{1}{z}(e^z-1) - \frac{n}{z}K_n(z)&\quad\mbox{if $n$ is multiple of $4$}\\
    \frac{1}{z}(e^z+1) - \frac{n}{z}K_n(z)&\quad\mbox{else}
\end{cases}\\
&= \frac{1}{z}(e^z-\cos(\frac{n\pi}{2})) - \frac{n}{z}K_n(z).
\end{align*}
Thus, $K_n(z)$ satisfies the recurrence relation
\begin{align*}
K_0(z) &= 0\\
K_1(z) &= \frac{e^z-1}{z}\\
K_{n}(z) &= K_{n-2}(z) + 2\left(\frac{e^z-\cos(\frac{(n-1)\pi}{2})}{z} - \frac{n-1}{z}K_{n-1}(z)\right),\quad n\geq 2.
\end{align*}
Hence, for $c\in\mathbb{C}\setminus\{0\}$,
\begin{align*}
\tilde{T}_n(c) &= \frac{1}{ic}\left(n\int_0^{\frac{\pi}{2}}e^{-ic\cos\theta}\sin(n\theta)\,d\theta + \cos(\frac{n\pi}{2})- e^{-ic}\right)\\
&= \frac{1}{ic}\left(nK_n(-ic) + \cos(\frac{n\pi}{2}) - e^{-ic}\right).
\end{align*}
...
\frac{1}{2}&\quad\mbox{if $n=1$}\\
\frac{n}{n^2-1}\sin(\frac{n\pi}{2})-\frac{1}{n^2-1}&\quad\mbox{if $n\geq 2$}
\end{cases},&\quad\mbox{$c=0$}\\
&= \frac{1}{ic}\left(nK_n(-ic) + \cos(\frac{n\pi}{2}) - e^{-ic}\right),&\quad\mbox{$c\neq 0$},
\end{alignat*}
where
\begin{align*}
K_0(z) &= 0\\
...
Since the Chebyshev coefficients are given on $[-1,1]$, we write 
\[f(x)=\sum_{n=0}^N a_n T_n(g(x))\]
where $g(x):[0,1]\to [-1,1]$ is an interval shift function given by $g(x)=2x-1$. As before, we wish to compute
...
\end{align*}
Then
\[q(t) = \sum_{n=0}^N b_n \cos(n\cdot \cos^{-1}(t)),\]
and so 
\begin{align*}
    \int_{-1}^1 e^{-ict} q(t)\,dt &= \int_{-1}^1 e^{-ict} \sum_{n=0}^N b_n \cos(n\cdot \cos^{-1}(t))\,dt \\
    &= \sum_{n=0}^N b_n \int_{-1}^1 e^{-ict}\cos(n\cdot \cos^{-1}(t))\,dt.
\end{align*}
With the change of variable $t=\cos\theta$,
\begin{align*}
    \int_{-1}^1 e^{-ict}\cos(n\cdot \cos^{-1}(t))\,dt &= \int_{-1}^1 e^{-ic\cos\theta}\cos(n\cdot \cos^{-1}(\cos\theta))\,d\cos\theta\\
