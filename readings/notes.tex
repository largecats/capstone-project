...
Find a basis for $\mathcal{V}$ by building a maximally linearly independent set adding one vector at a time. 

\begin{algorithm}[H]
    \caption{Finding a basis.}\label{algo:finding a basis}
    \KwData{An arbitrary $v_1\in\mathcal{V}$}
...
\unsure[inline]{Extending $U_1,\ldots,U_n$ to $U_1,\ldots,U_{2n}$ is equivalent to imbedding $(M:N)$ in a $2n\times 2n$ nonsingular matrix. Is there a better way than search-and-check?}
\end{document}
